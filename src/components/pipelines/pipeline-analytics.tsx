    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip={t('analytics.tooltipTotalDeals')}
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label={t('analytics.pipelineValue')}
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip={t('analytics.tooltipPipelineValue')}
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label={t('analytics.avgDealSize')}
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip={t('analytics.tooltipAvgDealSize')}
        />
        <Metric
        ...
        <Metric
          ...
          tooltip={t('analytics.tooltipWonThisMonth')}
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label={t('analytics.lostThisMonth')}
          value={String(stats.lostThisMonth)}
          tooltip={t('analytics.tooltipLostThisMonth')}
        />
      </div>
    </TooltipProvider>
  );
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <button
                type="button"
                aria-label={t('analytics.ariaHowCalculated').replace('{label}', label)}
