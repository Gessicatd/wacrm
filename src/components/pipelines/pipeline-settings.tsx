...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t('pipelines.toastSaveFailed'));
      return;
    }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t('pipelines.toastAddStageFailed'));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t('pipelines.toastRemoveStageDeals'));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t('pipelines.toastDeleteStageFailed'));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t('pipelines.toastDeletePipelineFailed'));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('pipelines.managePipelines')}</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? "Deleting..." : t('pipelines.deletePipeline')}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              ...
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t('pipelines.stages')}</Label>
                <DndContext
                  sensors={sensors}
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t('pipelines.pickColor').replace('{color}', color)}
                    />
                  ))}
                </div>
