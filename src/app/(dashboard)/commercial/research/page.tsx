...
export default function ResearchCenterPage() {
  ...
  useEffect(() => {
    void fetch('/api/consulting/projects', { cache: 'no-store' })
      ...
      .then(({ response, body }) => {
        ...
        const selected = loaded.find((project) => project.id === saved) ?? loaded[0];
        if (selected) {
          setProjectId(selected.id);
          window.localStorage.setItem('consulting_active_project', selected.id);
          void loadProjectData(selected.id);
        }
      });
  }, [loadProjectData]);
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-8">
    ...
    <section className={box}><label className="text-sm font-semibold">Empresa/projeto ativo<select value={projectId} onChange={(event) => { setProjectId(event.target.value); window.localStorage.setItem('consulting_active_project', event.target.value); void loadProjectData(event.target.value); }} className="border-input bg-background mt-2 h-10 w-full rounded-md border px-3"><option value="">Selecione uma empresa/projeto</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label></section>
