...
export default function CommercialAgentsPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-8">
    ...
    <header className="rounded-2xl border border-border bg-card p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="flex items-start gap-3"><Bot className="mt-1 h-6 w-6 text-primary" /><div><p className="text-sm font-medium text-primary">Agentes autônomos da metodologia</p><h1 className="mt-1 text-2xl font-bold">Inteligência com responsabilidade</h1><p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">Cada agente tem uma função, uma fonte de conhecimento, ferramentas permitidas e um limite de autonomia. O sistema recomenda primeiro; a execução automática só acontece quando for segura e autorizada.</p></div></div><div className="rounded-xl bg-primary/5 px-4 py-3 text-sm"><p className="text-xs text-muted-foreground">Prontos para validação</p><p className="mt-1 text-2xl font-bold text-primary">{ready}/{COMMERCIAL_AGENTS.length}</p></div></div>
...
function Safety({ icon: Icon, title, text }: { icon: typeof ShieldCheck; title: string; text: string }) { return <div className="rounded-xl border border-border bg-muted/30 p-3"><Icon className="h-4 w-4 text-primary" /><p className="mt-2 text-xs font-semibold">{title}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{text}</p></div> }
function Roadmap({ n, title, text }: { n: string; title: string; text: string }) { return <div className="rounded-xl bg-muted/40 p-4"><span className="text-xs font-bold text-primary">{n}</span><h3 className="mt-2 text-sm font-semibold">{title}</h3><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{text}</p></div> }
