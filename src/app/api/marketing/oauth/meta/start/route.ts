...
import { createOAuthState, metaAuthorizeUrl, providerEnv } from '@/lib/marketing/oauth'

export async function GET() {
  try {
    const { accountId } = await getCurrentAccount()
    const env = providerEnv('meta')
    if (!env.id || !env.redirect) return NextResponse.json({ error: 'Meta OAuth ainda não foi configurado no servidor.' }, { status: 503 })
